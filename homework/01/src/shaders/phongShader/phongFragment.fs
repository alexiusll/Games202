...
void uniformDiskSamples(const in vec2 randomSeed) {
  // ? 这个地方生成随机数不是很明确
  // float randNum = random(randomSeed);
  float sampleX = rand_1to1(0.1);
...
//比如2048像素的你就可以设置一个unit offset=1/2048
//他就会在某个像素的周围，按照filter size为半径找随机采样点
float PCF(sampler2D shadowMap, vec4 coords) {
  ...
  for (int i = 0; i < NUM_SAMPLES; i++) {
    ...
    // 不使用pack
    // float closesDepth = texture2D(shadowMap, texcoords).x;
